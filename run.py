import requests
import re
import urllib.parse as parse
from .urltool import d_url_comments

headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36',
    'cookie': 'Hm_lvt_276e4ec7ba0aa6e4db3b46c40cde6e63=1540823733; rand=7570; memcached_zz=383531_1528982984127265461nMg56is6Oj6E5bR4YNYroNtlxdZakXSX462knrxMkmKJbrtCxL5Lcqq84UmEBl5A3EWvOgtJfmc8qIfNQJuZw63BKbbtYueWoRCleHqbVTPnZ; dang_username=zaccc; JSESSIONID=A9F00AE476CDEA4BE4DD5DD7DF6817D6; DWRSESSIONID=D3dTxqr639g6YPSb4tTZAF5g1rm; Hm_lpvt_276e4ec7ba0aa6e4db3b46c40cde6e63=1540823982'
}

r = requests.Session()
r.headers = headers


def post_all(num, id, jid):
    def post_1(num):
        url = 'http://www.attop.com/js/ajax/call/plaincall/__System.pageLoaded.dwr'
        params = {
            "callCount": 1,
            "windowName": "",
            "c0-scriptName": "__System",
            "c0-methodName": "pageLoaded",
            "c0-id": 0,
            "batchId": 0,
            "instanceId": 0,
            "page": f"%2Fwk%2Fmedia_pop.htm%3Fid%3D{num}",
            "scriptSessionId": "D3dTxqr639g6YPSb4tTZAF5g1rm/Pe5h1rm-A4N5tX3td"
        }
        try:
            r.post(url=url, data=params)
        except Exception as e:
            print(e)

    def post_2(num):
        url = 'http://www.attop.com/js/ajax/call/plaincall/zsClass.dotAjax.dwr'
        params = {'callCount': 1, 'windowName': '', 'c0-scriptName': 'zsClass', 'c0-methodName': 'dotAjax', 'c0-id': 0,
                  'c0-param0': 'string:doWkMediaPj', 'c0-e1': f'number:{num}', 'c0-e2': 'number:3',
                  'c0-param1': 'Object_Object:{id:reference:c0-e1, type:reference:c0-e2}',
                  'c0-param2': 'string:doWkMediaPj', 'batchId': 1, 'instanceId': 0,
                  'page': f'%2Fwk%2Fmedia_pop.htm%3Fid%3D{num}',
                  'scriptSessionId': 'D3dTxqr639g6YPSb4tTZAF5g1rm/Pe5h1rm-A4N5tX3td'}
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36',
            'cookie': 'Hm_lvt_276e4ec7ba0aa6e4db3b46c40cde6e63=1540823733; rand=7570; memcached_zz=383531_1528982984127265461nMg56is6Oj6E5bR4YNYroNtlxdZakXSX462knrxMkmKJbrtCxL5Lcqq84UmEBl5A3EWvOgtJfmc8qIfNQJuZw63BKbbtYueWoRCleHqbVTPnZ; dang_username=zaccc; JSESSIONID=A9F00AE476CDEA4BE4DD5DD7DF6817D6; DWRSESSIONID=D3dTxqr639g6YPSb4tTZAF5g1rm; Hm_lpvt_276e4ec7ba0aa6e4db3b46c40cde6e63=1540823982',
            'Referer': f'http://www.attop.com/wk/media_pop.htm?id={num}'
        }
        r.headers = headers

        try:
            r.post(url=url, data=params)
        except Exception as e:
            print(e)

    def post_3():
        url = 'http://www.attop.com/js/ajax/call/plaincall/zsClass.commonAjax.dwr'
        params = {'callCount': 1, 'windowName': '', 'c0-scriptName': 'zsClass', 'c0-methodName': 'commonAjax',
                  'c0-id': 0,
                  'c0-param0': 'string:getTopDhNum', 'c0-param1': 'Object_Object:{}',
                  'c0-param2': 'string:doGetTopDhNum',
                  'batchId': 10, 'instanceId': 0, 'page': '%2Fuser%2Fstudy.htm',
                  'scriptSessionId': 'D3dTxqr639g6YPSb4tTZAF5g1rm/eUbg1rm-btMEtv2Ij'}

        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36',
            'cookie': 'Hm_lvt_276e4ec7ba0aa6e4db3b46c40cde6e63=1540823733; rand=7570; memcached_zz=383531_1528982984127265461nMg56is6Oj6E5bR4YNYroNtlxdZakXSX462knrxMkmKJbrtCxL5Lcqq84UmEBl5A3EWvOgtJfmc8qIfNQJuZw63BKbbtYueWoRCleHqbVTPnZ; dang_username=zaccc; JSESSIONID=A9F00AE476CDEA4BE4DD5DD7DF6817D6; DWRSESSIONID=D3dTxqr639g6YPSb4tTZAF5g1rm; Hm_lpvt_276e4ec7ba0aa6e4db3b46c40cde6e63=1540823982',
            'Referer': 'http://www.attop.com/user/study.htm'
        }
        r.headers = headers

        try:
            r.post(url=url, data=params)
        except Exception as e:
            print(e)

    def post_4(id, jid):
        url = 'http://www.attop.com/js/ajax/call/plaincall/zsClass.commonAjax.dwr'
        params = {'callCount': 1, 'windowName': '', 'c0-scriptName': 'zsClass', 'c0-methodName': 'commonAjax',
                  'c0-id': 0,
                  'c0-param0': 'string:getAjaxList', 'c0-e1': f'string:id%3D{id}%26jid%3D{jid}',
                  'c0-e2': 'string:learn.htm',
                  'c0-e3': 'number:1', 'c0-e4': 'string:showajaxinfo',
                  'c0-param1': 'Object_Object:{param:reference:c0-e1, pagename:reference:c0-e2, currentpage:reference:c0-e3, showmsg:reference:c0-e4}',
                  'c0-param2': 'string:doGetAjaxList', 'batchId': 5, 'instanceId': 0,
                  'page': f'%2Fwk%2Flearn.htm%3Fid%3D{id}%26jid%3D{jid}',
                  'scriptSessionId': 'D3dTxqr639g6YPSb4tTZAF5g1rm/PZ2h1rm-Fzku8t615'}

        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36',
            'cookie': 'Hm_lvt_276e4ec7ba0aa6e4db3b46c40cde6e63=1540823733; rand=7570; memcached_zz=383531_1528982984127265461nMg56is6Oj6E5bR4YNYroNtlxdZakXSX462knrxMkmKJbrtCxL5Lcqq84UmEBl5A3EWvOgtJfmc8qIfNQJuZw63BKbbtYueWoRCleHqbVTPnZ; dang_username=zaccc; JSESSIONID=A9F00AE476CDEA4BE4DD5DD7DF6817D6; DWRSESSIONID=D3dTxqr639g6YPSb4tTZAF5g1rm; Hm_lpvt_276e4ec7ba0aa6e4db3b46c40cde6e63=1540823982',
            'Referer': f'http://www.attop.com/wk/learn.htm?id={id}&jid={jid}'
        }
        r.headers = headers

        try:
            r.post(url=url, data=params)
        except Exception as e:
            print(e)

    post_1(num)
    post_2(num)
    post_3()
    post_4(id, jid)
    print(f'- 评价{num}成功')


def get_nums(id, jid):
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36',
        'cookie': 'Hm_lvt_276e4ec7ba0aa6e4db3b46c40cde6e63=1540823733; rand=7570; memcached_zz=383531_1528982984127265461nMg56is6Oj6E5bR4YNYroNtlxdZakXSX462knrxMkmKJbrtCxL5Lcqq84UmEBl5A3EWvOgtJfmc8qIfNQJuZw63BKbbtYueWoRCleHqbVTPnZ; dang_username=zaccc; JSESSIONID=A9F00AE476CDEA4BE4DD5DD7DF6817D6; DWRSESSIONID=D3dTxqr639g6YPSb4tTZAF5g1rm; Hm_lpvt_276e4ec7ba0aa6e4db3b46c40cde6e63=1540823982',
        'Referer': f'http://www.attop.com/wk/learn.htm?id={id}&jid={jid}'
    }
    r.headers = headers
    params = {'callCount': 1, 'windowName': '', 'c0-scriptName': 'zsClass', 'c0-methodName': 'commonAjax',
              'c0-id': 0, 'c0-param0': 'string:getAjaxList', 'c0-e1': f'string:id%3D{id}%26jid%3D{jid}',
              'c0-e2': 'string:learn.htm', 'c0-e3': 'number:1', 'c0-e4': 'string:showajaxinfo',
              'c0-param1': 'Object_Object:{param:reference:c0-e1, pagename:reference:c0-e2, currentpage:reference:c0-e3, showmsg:reference:c0-e4}',
              'c0-param2': 'string:doGetAjaxList', 'batchId': 2, 'instanceId': 0,
              'page': f'%2Fwk%2Flearn.htm%3Fid%3D{id}%26jid%3D{jid}',
              'scriptSessionId': 'D3dTxqr639g6YPSb4tTZAF5g1rm/7FXQ3rm-rPukkaJT5'}

    url = 'http://www.attop.com/js/ajax/call/plaincall/zsClass.commonAjax.dwr'
    res = r.post(url, data=params)
    # while len(res.text)<200:
    #     res = r.post(url)
    use = res.text.encode('utf8').decode('unicode_escape')
    # print(use)
    all_num = re.findall(r'media_([0-9]*)', use)
    print(all_num)
    return all_num


if __name__ == '__main__':
    urls = d_url_comments()
    for each in urls:
        id = parse.parse_qs(parse.urlparse(each).query)['id'][0]
        jid = parse.parse_qs(parse.urlparse(each).query)['jid'][0]
        for num in get_nums(id, jid):
            post_all(int(num), id, jid)
